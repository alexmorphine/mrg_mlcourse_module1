...
X_train, X_val, y_train, y_val = train_test_split(pics, labels, test_size=0.2, random_state=42)
mean = X_train.mean(axis=0)
std = X_train.std(axis=0)
idx = std.nonzero()
X_train[:, idx] = (X_train[:, idx] - mean[idx])/std[idx]
X_val[:, idx] = (X_val[:, idx] - mean[idx])/std[idx]
...
W, b = train(X_train, y_train, epoches=1000)
